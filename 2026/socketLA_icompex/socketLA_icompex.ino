...
#define Client_Id   "socketLAkksjkksjkolejKomunitiSeberangJaya0001"
...
String Socket01status="OFF";
String Socket02status="OFF";
//----------------------------------- end -------------------------------------------------------------
...
void connect() {
  ...
  Serial.println("\nconnected!");
  client.subscribe("socketLAkksjkksjkksj/socket01");
  client.subscribe("socketLAkksjkksjkksj/socket02");
  client.subscribe("socketLAkksjkksjkksj/buzzer");
...
void messageReceived(String &topic, String &payload) {
  ...
  if (String(topic) == "socketLAkksjkksjkksj/socket01") {
 ...
 if (String(topic) == "socketLAkksjkksjkksj/socket02") {
    Serial.print("Changing output to ");
    if(payload == "on"){
      Serial.println("on");
  ...
  if (String(topic) == "socketLAkksjkksjkksj/buzzer") {
    Serial.print("Changing output to ");
    if(payload == "on"){
      Serial.println("on");
      digitalWrite(buzzer,HIGH);
      client.publish("socketLAkksjkksjkksj/buzzer/status", "RELAY 2 ON");
    }
    else if(payload == "off"){
      Serial.println("off");
      digitalWrite(buzzer,LOW);
      client.publish("socketLAkksjkksjkksj/buzzer/status", "RELAY 2 OFF");
    }
  }
...
// publish a message roughly every second.
  ...
  lastMillis = millis();
   ...
   //Serial.println(dataLDR);
    client.publish("socketLAkksjkksjkksj/ldr", String(stableLightValue));  
    //----------------------------------------------------------------------------

    //------SENSOR 2--------------------------------------------------------------
    sensors.requestTemperatures();       // arahan baca suhu
    dataSuhuC = sensors.getTempCByIndex(0);  // nilai suhu disimpan dalam pembolehubah tempC
    Serial.print("Temperature: ");
    Serial.print(dataSuhuC);    
    Serial.println("°C");
    client.publish("socketLAkksjkksjkksj/suhu", String(dataSuhuC,0));
...
//------------------------------------------
 ...
 display.clearDisplay();
  ...
  display.println(stableLightValue);
  display.setCursor(0,30);
  display.println(" Suhu  : ");
  display.setCursor(60,30);
  display.println(dataSuhuC,0);
  display.setCursor(0,40);
  display.println("Socket 1: ");
  display.setCursor(60,40);
  display.println(Socket01status);
  display.setCursor(0,50);
  display.println("Socket 2: ");
  display.setCursor(60,50);
  display.println(Socket02status);
//  display.setCursor(40, 20);
  display.display();
  }
 //-----------end ----------------------------------
